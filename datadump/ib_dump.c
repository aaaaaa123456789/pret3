...
void dump_incbin_as_binary (struct incbin * incbin, const unsigned char * data) {
  ...
  if (!fp) return;
  if (fwrite(data, 1, incbin -> length, fp) != incbin -> length) puts("err: failed writing data to file");
  fclose(fp);
}
...
void dump_data_line_as_text (FILE * out, const unsigned char * data, unsigned offset, unsigned length) {
  ...
  for (; p < 16; p ++) fputs("   ", out);
  fputs("  ", out);
  for (p = 0; p < amount; p ++) {
    c = data[offset + p];
    if ((previewable[c >> 3] >> (c & 7)) & 1)
      fputs(text_table[c], out);
    else
      putc('@', out);
  }
  putc('\n', out);
